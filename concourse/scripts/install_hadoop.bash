...
function install_hadoop_single_cluster() {
    ...
    ssh ${SSH_OPTS} root@edw0 "
        ...
        sed -i -e 's/localhost/${hadoop_ip}/g' /etc/hive/conf/hive-site.xml &&
        sed -i -e 's/0.0.0.0/${hadoop_ip}/g'   /etc/hadoop/conf/core-site.xml &&
        sed -i -e 's/0.0.0.0/${hadoop_ip}/g'   /etc/hadoop/conf/hdfs-site.xml &&
        sed -i -e 's/0.0.0.0/${hadoop_ip}/g'   /etc/hadoop/conf/yarn-site.xml &&
        ...
        export HBASE_ROOT=/singlecluster/hbase &&
        export HIVE_ROOT=/singlecluster/hive &&
        export ZOOKEEPER_ROOT=/singlecluster/zookeeper &&
        export PATH=\$PATH:\${GPHD_ROOT}/bin:\${HADOOP_ROOT}/bin:\${HBASE_ROOT}/bin:\${HIVE_ROOT}/bin:\${ZOOKEEPER_ROOT}/bin &&

        groupadd supergroup && usermod -a -G supergroup gpadmin &&
        setup_impersonation /singlecluster &&
        start_hadoop_services /singlecluster"
}
